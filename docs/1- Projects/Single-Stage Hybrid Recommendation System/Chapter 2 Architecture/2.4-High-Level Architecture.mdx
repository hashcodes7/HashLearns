...
## 25. Production Data Flow (PostgreSQL & pgvector Pipeline)

Our system leverages **PostgreSQL** and the **pgvector** extension to completely unify storage and similarity search. This eliminates the need to keep large `.pkl` indexes in memory and ensures our API remains extremely lightweight.

### Architecture Diagram

```mermaid
flowchart TD
    subgraph Online Serving [Recommendation API]
        APP[Customer Application] -->|GET /recommendations| API[api/routes.py]
        API -->|Calls| REC[core/recommender.py]
        REC -->|Queries| DB_PG[(PostgreSQL pgvector)]
        API -->|POST /events| DB_EV[(events.db SQLite / Postgres)]
    end

    subgraph Offline Training [Background Pipeline]
        DB_EV -->|Reads Events| TRAIN[core/trainer.py]
        TRAIN -->|Calculates User Vector| TRAIN
        TRAIN -->|Upserts User Vector| DB_PG
    end
```
...
1. **FastAPI Event Logging (`api/routes.py`)**: 
   Exposes a `POST /events` (or similar logging mechanisms) to capture user interactions (clicks, views) into an `events.db` database.

2. **Embeddings Updater (`core/trainer.py`)**: 
   The `update_user_embeddings()` method fetches events from the events database, recalculates dynamic user vectors by averaging product embeddings (fetched from PostgreSQL), and upserts them directly into the PostgreSQL `user_embeddings` table.

3. **Database Layer (`core/database.py`)**: 
   Handles PostgreSQL connections using `psycopg2`. It utilizes `pgvector`'s `<=>` (cosine distance) operator to perform the KNN search natively inside the database, totally eliminating the need for `scikit-learn` or `.pkl` files.

4. **Pipeline Orchestrator (`main.py`)**: 
   The FastAPI app allows triggering the `core/trainer.py` pipeline as a background task to keep user vectors fresh based on real-time events.
