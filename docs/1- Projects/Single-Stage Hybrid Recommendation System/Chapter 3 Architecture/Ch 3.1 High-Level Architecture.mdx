...
## 25. Production Data Flow (Offline KNN Training Pipeline)

A realistic system separates online serving from offline processing. Our implementation fully decouples the heavy KNN indexing from the FastAPI server.

### Architecture Diagram

```mermaid
flowchart TD
    subgraph Online Serving [Enterprise-Recommendation-system-Recommender]
        APP[Customer Application] -->|GET /recommendations| API[api/routes.py]
        API -->|Instantiates| REC[core/recommender.py: HybridRecommender]
        REC -->|Loads| KNN[core/knn_model.pkl]
        REC -->|Loads| UV[core/user_vectors.pkl]
        API -->|POST /events| DB[(events.db)]
    end

    subgraph Offline Training [Enterprise-Recommendation-system-Trainer]
        DB -->|Reads Events| UE[update_embeddings.py: update_user_embeddings]
        UE -->|Recalculates & Saves| UV
        PE[core/data.py: Product Vectors] -->|Reads| UKNN[update_knn_model.py: update_knn]
        UKNN -->|Fits & Saves NearestNeighbors| KNN
        
        TRAIN[train.py: run_pipeline] -->|1. Calls| UE
        TRAIN -->|2. Calls| UKNN
    end
```
...
1. **FastAPI Event Logging (`api/routes.py`)**: 
   Exposes a `POST /events` endpoint to capture user interactions (clicks, views) into an SQLite database (`events.db`).

2. **Embeddings Updater (`update_embeddings.py`)**: 
   The `update_user_embeddings()` method fetches events from `events.db`, merges them with `core/data.py`'s `users_history`, recalculates the dynamic user vectors by averaging product embeddings, and saves them to `user_vectors.pkl`.

3. **KNN Model Updater (`update_knn_model.py`)**: 
   The `update_knn()` method strictly fits the `sklearn.neighbors.NearestNeighbors` algorithm on the product catalog and saves it as `knn_model.pkl`.

4. **Pipeline Orchestrator (`train.py`)**: 
   The `run_pipeline()` method acts as the master trigger that runs `update_embeddings` followed by `update_knn_model`.

This architecture ensures the API memory footprint remains tiny and request latency is minimized, as it simply retrieves pre-built indexes rather than calculating embeddings on the fly.
